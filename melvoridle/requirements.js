...
class CompletionRequirement extends GameRequirement {
    ...
    getNodes(imageClass) {
        return [
            templateLangString(`MENU_TEXT_${this.namespace.isModded ? 'REQUIRES_COMPLETION_MOD' : `REQUIRES_COMPLETION_${this.namespace.name}`}`, { percent: `${this.percent}`, modName: this.namespace.displayName }),
        ];
    }
}
...
class SkillLevelRequirement extends GameRequirement {
    ...
    getNodes(imageClass) {
        return templateLangStringWithNodes('MENU_TEXT_REQUIRES_SKILL_LEVEL', { skillImage: this.createImage(this.skill.media, imageClass) }, { level: `${this.level}` });
    }
}
...
class AbyssalLevelRequirement extends GameRequirement {
    ...
    notifyFailure() {
        if (!this.skill.isUnlocked) {
            ...
            notifyPlayer(this.skill, templateLangString('ABYSSAL_LEVEL_REQUIRED_TO_DO_THAT', { level: `${this.level}`, skillName: this.skill.name }), 'danger');
        }
    }
    _assignHandler(handler) {
    ...
    getNodes(imageClass) {
        return templateLangStringWithNodes('REQUIRES_ABYSSAL_LEVEL', { skillImage: this.createImage(this.skill.media, imageClass) }, { level: `${this.level}` });
    }
}
...
class DungeonRequirement extends GameRequirement {
    ...
    getNodes(imageClass) {
        return templateLangStringWithNodes(`MENU_TEXT_${this.count > 1 ? 'COMPLETE_DUNGEON_TIMES' : 'COMPLETE_DUNGEON_ONCE'}`, { dungeonImage: this.createImage(this.dungeon.media, imageClass) }, { dungeonName: this.dungeon.name, count: `${this.count}` });
    }
}
/** Requires that the given dungeon has been completed count times */
class StrongholdRequirement extends GameRequirement {
    ...
    getNodes(imageClass) {
        return templateLangStringWithNodes(`MENU_TEXT_${this.count > 1 ? 'COMPLETE_DUNGEON_TIMES' : 'COMPLETE_DUNGEON_ONCE'}`, { dungeonImage: this.createImage(this.stronghold.media, imageClass) }, { dungeonName: this.stronghold.name, count: `${this.count}` });
    }
}
...
class AbyssDepthRequirement extends GameRequirement {
    ...
    getNodes(imageClass) {
        return templateLangStringWithNodes(`MENU_TEXT_${this.count > 1 ? 'COMPLETE_THE_ABYSS_TIMES' : 'COMPLETE_THE_ABYSS_ONCE'}`, { depthImage: this.createImage(this.depth.media, imageClass) }, { depthName: this.depth.name, count: `${this.count}` });
    }
}
...
class ShopPurchaseRequirement extends GameRequirement {
    ...
    getNodes(imageClass) {
        return templateLangStringWithNodes('MENU_TEXT_SHOP_ITEM_PURCHASED', { shopImage: this.createImage(this.purchase.media, imageClass) }, { shopName: this.purchase.name });
    }
}
...
class SlayerItemRequirement extends GameRequirement {
    ...
    notifyFailure() {
        notifyPlayer(this.game.attack, templateLangString('TOASTS_ITEM_EQUIPPED_REQUIRED', { itemName: this.item.name }), 'danger');
    }
    _assignHandler(handler) {
...
class ItemFoundRequirement extends GameRequirement {
    ...
    notifyFailure() {
        imageNotify(this.item.media, templateLangString('TOASTS_ITEM_FOUND_REQUIRED', { itemName: this.item.name }), 'danger');
    }
    getHandler(callback) {
        return (e) => {
            if (e.item === this.item)
    ...
    getNodes(imageClass) {
        return templateLangStringWithNodes('MENU_TEXT_FIND_ITEM', { itemImage: this.createImage(this.item.media, imageClass) }, { itemName: this.item.name });
    }
}
class MonsterKilledRequirement extends GameRequirement {
    ...
    getNodes(imageClass) {
        return templateLangStringWithNodes(`MENU_TEXT_${this.count > 1 ? 'DEFEAT_MONSTER_TIMES' : 'DEFEAT_MONSTER_ONCE'}`, { monsterImage: this.createImage(this.monster.media, imageClass) }, { monsterName: this.monster.name, count: `${this.count}` });
    }
}

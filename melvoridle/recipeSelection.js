...
class RecipeOptionElement extends HTMLElement {
    ...
    setLocked(recipe, skill) {
        ...
        levelSpan.append(...templateLangStringWithNodes('MENU_TEXT_UNLOCKED_AT', { skillImage: skillImage.cloneNode() }, { level: `${recipe.level}` }));
        if (recipe.abyssalLevel > 0) {
            const levelSpan = createElement('span', {
                className: skill.abyssalLevel >= recipe.abyssalLevel ? 'text-success' : 'text-danger',
                parent: this.locked,
            });
            levelSpan.append(...templateLangStringWithNodes('UNLOCKED_AT_ABYSSAL_LEVEL', { skillImage: skillImage.cloneNode() }, { level: `${recipe.abyssalLevel}` }));
        }
    }
    setUnlocked(recipe) {
